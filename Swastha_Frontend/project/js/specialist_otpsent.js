...
async function profile() {
    try {
        let response = await fetch(`http://localhost:8080/fetchspecialistByemail?email=${user_email}`, {
            method: "GET",
        });
...
verify[0].addEventListener("click", (e) => {
    ...
    if (check_otp.value.trim() === otp) {
        ...
        window.location.href = "http://127.0.0.1:5500/project/html/specialst_cpwd.html";
    } else {
        alert("invalid otp")
    }
